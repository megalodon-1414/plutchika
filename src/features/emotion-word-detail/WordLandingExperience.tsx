...
function GemButtonContent({
  ...
  return (
    <>
      ...
      {!selected && (
        <span className="word-landing__choice-callout" aria-hidden>
          <svg
            className="word-landing__choice-leader"
            viewBox="0 0 56 28"
          ...
          >
            {side === 'left' ? (
              /* 左ジェム → 右下のラベル */
              <>
                <path d="M2 4 L20 22 L54 22" />
                <circle cx="2" cy="4" r="2.1" />
              </>
            ) : (
              /* 右ジェム → 左下のラベル */
              <>
                <path d="M54 4 L36 22 L2 22" />
                <circle cx="54" cy="4" r="2.1" />
              </>
            )}
          </svg>
...
export function WordLandingExperience({
  ...
  /** Mapに戻る：人物が乗り込んでいるあいだ */
  const [boarding, setBoarding] = useState(false);
  /** Mapに戻る：ロケット離陸演出中。終わると /telescope へ遷移する */
  ...
  const surface = useMemo(() => {
    ...
    const horizonRatio = wordPlanetHorizonRatio(vw);
    const surfaceBottomRatio = 1 - horizonRatio;
    const surfaceBottom = surfaceBottomRatio * vh; // CSSの --surface-bottom と対
    const centerBottom = surfaceBottom - radius; // 惑星中心のbottom座標(px)
    const isMobile = vw <= 640;
    /* PCは従来どおり。スマホは小さくして、画面下端から少し上に固定配置する */
    const gemSize = isMobile
      ? Math.round(Math.min(100, Math.max(76, vw * 0.21)))
      : Math.round(Math.min(200, Math.max(112, vw * 0.145)));
    const gemHalf = gemSize / 2;
    /* ジェムは画面内に収まる範囲で、できるだけ外側へ置く */
    const desiredGemX = vw * (isMobile ? 0.38 : 0.36);
    const maxByViewport = vw / 2 - gemHalf - (isMobile ? 4 : 16);
    const maxBySphere = radius * (isMobile ? 0.55 : 0.55);
    const gemX = Math.max(36, Math.min(desiredGemX, maxByViewport, maxBySphere));
    /* 左右ジェムは同じ距離。旗の植え位置は flagSpot 側で調整する */
    const gemXCompose = gemX;
    const gemXMeaning = gemX;
    /* PC: 星の円弧上。スマホ: 画面下から少し上げた位置（星の弧には載せない） */
    const gemBottom = isMobile
      ? Math.round(Math.max(52, Math.min(88, vh * 0.1)))
      : centerBottom + Math.sqrt(Math.max(0, radius ** 2 - gemX ** 2));
    return {
      vw,
      vh,
      radius,
  ...
  const gemAngleCompose = toPolar(surface.gemXCompose, surface.gemBottom).angle;
  const gemAngleMeaning = toPolar(surface.gemXMeaning, surface.gemBottom).angle;
  /** 人物の立ち位置（固定）。星が回ってもここから動かない */
  const personX = 73;
  /** 球体の回転量（画面上で時計回り正）。クリックされたジェムは必ず星の中央（頂点）に来る */
  const rotationDelta =
    rotationPanel === 'meaning' ? -gemAngleMeaning : rotationPanel === 'compose' ? gemAngleCompose : 0;
   ...
   * 3本目以降は左右交互に少しずつずらす。スマホではロケット寄りに近付けて刺す */
  const flagSpot = (index: number) => {
    const scale = Math.min(1, surface.radius / 520);
    const maxOffset = surface.radius * (surface.isMobile ? 0.28 : 0.5);
    const firstBase = surface.isMobile ? 150 : 240;
    const firstOffset = Math.min(maxOffset, firstBase * scale);
    ...
    const steps = surface.isMobile
      ? ([0, 30, 60, 15, 45, 75] as const)
      : ([0, 38, 76, 19, 57, 95] as const);
    const step = steps[pair % steps.length];
    const row = Math.floor(pair / steps.length) % 3;
    const offset = Math.min(maxOffset, (firstBase + step) * scale);
  ...
  /** 基準位置を球体の回転ぶんだけ回した配置（位置＋傾き）を返す。
   *  liftPxで半径方向に浮かせられる。pitch は奥行き回転（ロケットと同じ式） */
  const placeOnSurface = (
    x: number,
    bottomPx: number,
    liftPx = 0,
    yaw = planetRotationDelta,
    pitch = 0,
  ) => {
    const p = toPolar(x, bottomPx);
    const a = p.angle + yaw;
    const r = p.radius + liftPx;
    const radialY = r * Math.cos(a);
    return {
      x: r * Math.sin(a),
      bottom: surface.centerBottom + radialY * Math.cos(pitch),
      angleRad: a,
      /** 正で手前、負で奥（カメラが +Z にある前提） */
      depthZ: radialY * Math.sin(pitch),
    };
  };
  ...
  /** 人物の画面上の立ち位置。意味表示中はジェムの左側（画面左下）へ、
      ...
      旗を刺しに行くときは植え場所へ。刺したあともポール右側に留まる。
      Mapへ戻るときはロケット中央へ乗り込んで消える */
  const standingAtFlag =
    Boolean(focusedFlagPlace) && (flagsFocused || expandedFlagIndex >= 0);
  ...
  /** 惑星回転の歩行タイマーが終了しても、植え場所への移動中・帰り道・乗込中は歩行を継続する */
  ...
  /** Mapに戻る：パネル／旗を閉じ、人が乗り込んでからロケット離陸→黒フェードアウト→/telescope */
  const handleReturnToMap = () => {
    if (boarding || takingOff) {
      return;
    }
    if (activePanel !== null) {
      setActivePanel(null);
    ...
    const clearDepartureTimers = () => {
      if (boardTimerRef.current !== null) {
        window.clearTimeout(boardTimerRef.current);
        boardTimerRef.current = null;
      }
      if (takeoffTimerRef.current !== null) {
        window.clearTimeout(takeoffTimerRef.current);
    ...
    const startLaunch = () => {
      ...
      takeoffTimerRef.current = window.setTimeout(() => {
        navigate(ROUTES.telescopeSpace, {
          state: buildTelescopeRestoreState(plot),
        });
        takeoffTimerRef.current = null;
      }, reducedMotion ? 280 : ROCKET_TAKEOFF_TOTAL_MS + 120);
    };
  ...
  const selectFlag = (flagId: string) => {
    if (boarding || takingOff || planting !== null) {
      return;
    }
  ...
  const togglePanel = (panel: LandingPanel) => {
    ...
    // 人は旗の現在地から歩きで向かう（returningHome の transition を使う。false にするとワープする）。
  ...
  return (
    <main
      ...
      style={{
        ...
        /* PC=20% / スマホ=28%。wordPlanetHorizonRatio と必ず揃える */
        '--surface-bottom': `${((1 - wordPlanetHorizonRatio(surface.vw)) * 100).toFixed(0)}%`,
      } as CSSProperties}
    >
      <style>{`
        .word-landing {
          --landing-time: 3.3s;
          /* 地平線の既定値（インラインの --surface-bottom で上書き。PC=20% / スマホ=28%） */
          --surface-bottom: 20%;
          position: relative;
          width: 100%;
          height: 100%;
        ...
        .word-landing__exit-fade {
          ...
          opacity: 0;
          pointer-events: none;
          animation: wordLandingExitFade 0.75s ease-in 1.05s 1 forwards;
        }
        @keyframes wordLandingExitFade {
          from { opacity: 0; }
          to { opacity: 1; }
        }
        ...
        .word-landing__person {
          position: absolute;
          /* 旗Canvas(z=4)より手前。星の回転で旗が横切るとき人が旗の前を通って見えるようにする */
          z-index: 5;
          left: calc(50% + 58px);
          bottom: var(--surface-bottom);
          width: 30px;
        ...
        /* Mapへ戻る：ロケット中央へ歩きながら、徐々に薄くなって消える */
        .word-landing__person.is-boarding {
          /* personArrive の opacity 上書きを外し、乗込用フェードを再生 */
          animation: personBoardFade 0.9s linear forwards;
          transition:
            left .9s cubic-bezier(.35,.5,.3,1),
            bottom .5s cubic-bezier(.25,.6,.25,1);
        }
        @keyframes personBoardFade {
          0% {
            opacity: 1;
          }
          40% {
        ...
        .word-landing__person.is-boarding .word-landing__person-held-flag {
          opacity: 0;
          transition: opacity .35s ease;
        }
        .word-landing__person.is-boarding .word-landing__person-head {
          animation: personWalkHead .4s ease-in-out infinite;
        }
        .word-landing__person.is-boarding .word-landing__person-leg--left {
          animation: personWalkLegLeft .4s ease-in-out infinite;
        }
        .word-landing__person.is-boarding .word-landing__person-leg--right {
          animation: personWalkLegRight .4s ease-in-out infinite;
        }
        ...
        /* ラベルは枠なし。左ジェムは右下、右ジェムは左下へ引き出し線でつなぐ */
        .word-landing__choice-callout {
          position: absolute;
          top: auto;
          z-index: 1;
          display: flex;
        ...
        .word-landing__choice--left .word-landing__choice-callout {
          left: calc(100% - 12px);
          right: auto;
          bottom: -8%;
          flex-direction: row;
        }
        .word-landing__choice--right .word-landing__choice-callout {
          /* ジェムの左下から引き出し。値を大きくするほどラベルが右へ寄る */
          right: calc(100% - 88px);
          left: auto;
          bottom: -18%;
          flex-direction: row-reverse;
        }
        .word-landing__choice-leader {
          width: 52px;
          height: 26px;
          flex: 0 0 auto;
          overflow: visible;
          color: color-mix(in srgb, var(--planet-glow) 78%, #050508);
          filter: none;
        }
        .word-landing__choice-leader path {
          fill: none;
          stroke: currentColor;
          stroke-width: 1.5;
        ...
        .word-landing__choice-leader circle {
          fill: currentColor;
          filter: none;
        }
        .word-landing__choice-label {
          ...
          text-shadow:
            0 0 6px var(--planet-glow),
            0 0 14px var(--planet-glow),
            0 0 28px var(--planet-glow),
            0 1px 3px rgba(0, 0, 0, 0.55);
          filter: drop-shadow(0 0 10px var(--planet-glow));
        }
        .word-landing__choice--left .word-landing__choice-label {
          text-align: left;
        }
        .word-landing__choice--right .word-landing__choice-label {
          text-align: right;
        }
        ...
        .word-landing__planted {
          ...
          transform: translateX(-50%);
          transform-origin: 50% 100%;
          animation: flagPlant .5s cubic-bezier(.2,.9,.2,1) calc(var(--landing-time) + .6s) both;
          transition:
            left .8s cubic-bezier(.25,.6,.25,1),
            bottom .8s cubic-bezier(.25,.6,.25,1),
            transform .8s cubic-bezier(.25,.6,.25,1),
        ...
        .word-landing__planted-scroll {
          ...
          width: var(--flag-scroll-w, min(336px, 86.4vw));
      ...
      {(() => {
        ...
        const leftGemPlace = placeOnSurface(
          -surface.gemXCompose,
          surface.gemBottom,
          !flagCameraActive && rotationPanel === 'compose' ? gemLiftCompose : 0,
        );
        const rightGemPlace = placeOnSurface(
          surface.gemXMeaning,
          surface.gemBottom,
          !flagCameraActive && rotationPanel === 'meaning' ? gemLiftMeaning : 0,
        );
        return (
          <>
            ...
            <div
              ...
              style={{
                opacity: takingOff || rotationPanel !== null || flagFocusSpot !== null ? 0 : 1,
            ...
            <RocketModel
              ...
              heightPx={156}
              feetBottom={() => surface.surfaceBottom}
              surfaceAngleRad={takingOff ? 0 : planetRotationDelta}
              surfacePitchRad={
                takingOff
                  ? 0
                  : rotationPanel !== null || flagFocusSpot !== null
                    ? -0.5
                    : 0
              }
              takingOff={takingOff}
            ...
            <div
              ...
              style={{
                ...
                bottom: personBottom,
                transform: 'translateX(-50%)',
                opacity: takingOff ? 0 : undefined,
                pointerEvents: boarding || takingOff ? 'none' : undefined,
                '--held-flag': uiTheme.accent,
                /* 旗を刺しに行く間・戻る間・乗込中は歩幅に合わせて移動し、ワープに見せない */
                transition: boarding
                  ? undefined
                  : planting
      ...
      {flags.map((flag, index) => {
        ...
        // 旗は星の表面に固定。面内回転に加え、ジェム選択時の奥行き回転にも乗せる
        ...
        const flagScale = isExpanded ? 1 : receding ? Math.max(0.55, 1 + flagPlace.depthZ / 120) : 1;
        return (
          <div
            key={flag.id}
            role="button"
          ...
          >
            {showScroll && (
              <div
                ...
                onClick={(event) => event.stopPropagation()}
                style={{
                  '--flag-scroll-font': flagScrollFontSize(flag.sentence, surface.isMobile),
                } as CSSProperties}
              >
                <svg
      ...
      {/* 画面上で時計回り正の回転量を、Three.js（反時計回り正）の角度に変換して渡す。
          ...
          旗はロケットより手前の専用レイヤーで描画し、星に刺さった見え方は深度用の月で保つ */}
      ...
      <PlantedFlagsLayer
        ...
        flags={flags.map((flag, index) => {
          ...
          /* 3D旗は必ず星の半径上に置き、画面幅で極座標半径がズレて星から抜けるのを防ぐ。
             ロード時の旗はジェムと同じタイミングまで隠す。今刺した旗はすぐ生やす */
          return {
            id: flag.id,
            angleRad: polar.angle,
            radiusPx: surface.radius,
            selected: expandedFlagId === flag.id,
            appearDelaySec: isNew ? 0 : FLAG_LOAD_APPEAR_DELAY_SEC,
          };
        })}
      />
    </main>
