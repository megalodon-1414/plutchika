...
/** スマホ時：見出し（またはサブコピー）下端から本文上端までの隙間 */
const MOBILE_HEADING_BODY_GAP_PX = 22;
...
function WelcomePanelLayout({
  ...
  const stackGap = MOBILE_HEADING_BODY_GAP_PX;
  /** 見出しと「ぷるちか」の字間（かなり狭く） */
...
const PETAL_WHEEL_HTML_SIZE = 300;
...
function SplitGraphicPanelLayout({
  ...
  const bodyY =
    isMobile && headingHeight > 0
      ? headingY - headingHeight - MOBILE_HEADING_BODY_GAP_PX
      : skyHeight * 0.6;
